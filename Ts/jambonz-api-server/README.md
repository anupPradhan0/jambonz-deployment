...
After `npm run build`, apply schema upgrades with:

```
npm run upgrade-db
```

For development without a prior build, use `npm run upgrade-db:dev` (runs the TypeScript source via `tsx`).
...
For development with hot TypeScript execution (no `dist/`):
...
`npm start` runs `node dist/app.js` after `npm run build`.
