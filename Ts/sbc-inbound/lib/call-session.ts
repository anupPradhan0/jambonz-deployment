// @ts-nocheck — runtime-aligned with JS; explicit typing deferred (large dialog/RTP surface).
...
import type { JambonzSrfRequest, JambonzSrfResponse, SipDialog, SrfInstance } from '../types/jambonz-sip';

const {parseUri, stringifyUri, SipError} = require('drachtio-srf') as {
  parseUri: (u: string) => { host?: string; user?: string } | null;
  stringifyUri: (u: Record<string, unknown>) => string;
  SipError: new (status: number, reason?: string) => Error & { status?: number; reason?: string };
};
const debug = debugFactory('jambonz:sbc-inbound');
const { MediaPath } = constantsJson;
