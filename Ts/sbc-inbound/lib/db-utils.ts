// @ts-nocheck — ported from JS; narrow types incrementally if desired.
import assert from 'node:assert';
import CIDRMatcher = require('cidr-matcher');
import { normalizeDID } from './utils';
import type { Logger } from 'pino';
import type { SrfInstance } from '../types/jambonz-sip';

const {parseUri} = require('drachtio-srf') as { parseUri: (u: string) => { host?: string; user?: string } | null };
...
export default function dbUtilsFactory(srf: SrfInstance, logger: Logger) {
