...
import realtimeFactory from '@jambonz/realtimedb-helpers';
import {
  isValidIP,
  isDrained,
  getDrainedServers,
...
} from '../lib/cli/feature-server-config';
...
const { addToSet, removeFromSet, retrieveSet } = realtimeFactory(
  {
    host: JAMBONES_REDIS_HOST || 'localhost',
    port: parseInt(String(JAMBONES_REDIS_PORT || 6379), 10)
  },
  logger
);
...
function runCli(command: string) {
  return new Promise<{ stdout: string; stderr: string }>((resolve, reject) => {
    exec(`npm run cli ${command}`, { cwd: path.join(__dirname, '..') }, (err, stdout, stderr) => {
      if (err) {
...
test('setup feature server test data', async (t) => {
  try {
    ...
    t.fail(`Failed to setup test data: ${(err as Error).message}`);
    t.end();
  }
});
...
test('CLI integration tests', (t) => {
  ...
  connect(srf)
    ...
    .catch((err) => {
      ...
      if (srf) srf.disconnect();
      const e = err as { err?: Error; message?: string };
      t.fail(`CLI test failed: ${e.err ? e.err.message : e.message}`);
  ...
  connect(srf)
    ...
    .then(async () => {
      ...
      try {
        ...
        t.ok(
          (e.stderr?.includes('Invalid IP') ?? false) || (e.stdout?.includes('Invalid IP') ?? false),
          'drain rejects invalid IP'
        );
      ...
      try {
        ...
        const e = err as { stderr?: string; stdout?: string };
        t.ok(
          (e.stderr?.includes('Unknown command') ?? false) || (e.stdout?.includes('Unknown command') ?? false),
      ...
      try {
        ...
        const e = err as { stderr?: string; stdout?: string };
        t.ok(
          (e.stderr?.includes('requires server') ?? false) || (e.stdout?.includes('requires server') ?? false),
          'drain requires server argument'
        );
    ...
    .catch((err) => {
      ...
      if (srf) srf.disconnect();
      t.fail(`CLI error test failed: ${(err as Error).message}`);
      t.end();
    });
});
...
test('CLI config commands', (t) => {
  ...
  connect(srf)
    ...
    .catch((err) => {
      ...
      if (srf) srf.disconnect();
      const e = err as { err?: Error; message?: string };
      t.fail(`CLI config test failed: ${e.err ? e.err.message : e.message}`);
      if (err.stderr) console.log('stderr:', err.stderr);
      if (err.stdout) console.log('stdout:', err.stdout);
      t.end();
    });
...
test('cleanup feature server test data', async (t) => {
  try {
    ...
    t.fail(`Failed to cleanup test data: ${(err as Error).message}`);
    t.end();
  }
});
...
test('Feature Server Config utility functions', (t) => {
  ...
  connect(srf)
    ...
    .catch((err) => {
      ...
      if (srf) srf.disconnect();
      t.fail(`Feature server config test failed: ${(err as Error).message}`);
      t.end();
    });
});
...
test('Feature Server Config edge cases and error handling', (t) => {
  ...
  Promise.all([
    ...
    t.fail(`Edge case test failed: ${(err as Error).message}`);
    t.end();
  });
});