...
function connectAndSend(command: Record<string, unknown>) {
  return new Promise<Record<string, unknown>>((resolve, reject) => {
    ...
    socket.on('data', (data) => {
      try {
        const response = JSON.parse(data.toString().trim()) as Record<string, unknown>;
        socket.end();
        resolve(response);
      } catch {
        socket.end();
...
function validateArgs(args: string[], minCount: number, usage: string) {
  if (args.length < minCount) {
    exitWithError(`${usage}`);
  }
}

async function handleFeatureServerCommand(args: string[]) {
  ...
  switch (action) {
    case 'drain':
      validateArgs(args, 2, 'drain requires server IP address');
      {
        ...
        if (!drainResult.success) {
          exitWithError((drainResult.error as string) || 'Failed to drain server');
        }

        console.log(`✓ Drained ${drainResult.server}`);
        console.log(`Drained servers: [${(drainResult.drained as string[]).join(', ')}]`);
      }
      break;
    ...
    case 'undrain':
      validateArgs(args, 2, 'undrain requires server IP address');
      {
        ...
        if (!undrainResult.success) {
          exitWithError((undrainResult.error as string) || 'Failed to undrain server');
        }

        console.log(`✓ Undrained ${undrainResult.server}`);
        console.log(`Drained servers: [${(undrainResult.drained as string[]).join(', ')}]`);
      }
      break;
    ...
    case 'drained': {
      ...
      if (drainedResult.error) {
        exitWithError(drainedResult.error as string);
      }

      const drained = drainedResult.drained as string[];
      if (drained.length === 0) {
        console.log('No servers are currently drained');
      } else {
        console.log('Drained servers:');
        drained.forEach((s) => console.log(`  🔴 ${s}`));
      }
      break;
    }
    ...
    case 'active': {
      ...
      if (!activeResult.success) {
        exitWithError((activeResult.error as string) || 'Failed to get active servers');
      }

      const available = activeResult.available as string[];
    ...
    case 'list': {
      ...
      if (listResult.error) {
        exitWithError(listResult.error as string);
      }

      const servers = listResult.servers as Array<{ server: string; status: string }>;
      const drainedList = listResult.drained as string[];
      if (servers.length === 0) {
        console.log('No servers configured');
        if (drainedList.length > 0) {
          console.log('Orphaned drained servers:');
          drainedList.forEach((s) => console.log(`  🔴 ${s} (orphaned)`));
        }
      } else {
        console.log('Feature servers:');
        servers.forEach(({ server, status }) => {
...
async function handleConfigCommand(action: string, args: string[]) {
  switch (action) {
    case 'set':
      validateArgs(args, 2, 'set requires key and value');
      {
        ...
        if (setResult.error) {
          exitWithError(setResult.error as string);
        }
    ...
    case 'get':
      validateArgs(args, 1, 'get requires key');
      {
        ...
        if (getResult.error) {
          exitWithError(getResult.error as string);
        }
    ...
    case 'list': {
      ...
      const entries = Object.entries(listResult.config as Record<string, unknown>);
      if (entries.length === 0) {
