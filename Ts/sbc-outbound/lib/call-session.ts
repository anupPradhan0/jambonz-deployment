import { EventEmitter as Emitter, EventEmitter } from 'events';
import SrsClient = require('@jambonz/siprec-client-utils');
import {
  makeRtpEngineOpts,
  nudgeCallCounts,
  ...
  isValidDomainOrIP,
  removeVideoSdp,
  type RtpEngineOptsBundle
} from './utils';
import { forwardInDialogRequests } from 'drachtio-fn-b2b-sugar';
import debugFactory = require('debug');
import type { Logger } from 'pino';
import type { SipGatewayRow, VoipCarrierRow } from '@jambonz/db-helpers';
...
import type {
  ...
  SipDialog,
  SrfInstance
} from '../types/jambonz-sip';

/** In-dialog request option bags are not full Drachtio SrfRequest objects at runtime. */
type RawDialogRequest = Record<string, unknown>;
const asDialogRequest = (o: RawDialogRequest): DrachtioSrf.SrfRequest => o as unknown as DrachtioSrf.SrfRequest;

/** {@link SipDialog.on} overload resolution wrongly picks only `"update"` in strict mode. */
const dlgOn = (dlg: SipDialog, ev: string, fn: (...args: unknown[]) => void): void => {
  (dlg as EventEmitter).on(ev, fn);
};

type SrfReq = JambonzSrfRequest;
type SrfRes = JambonzSrfResponse;
...
interface RtpEngineClient {
  offer(opts: Record<string, unknown>): Promise<RtpEngineResult>;
  answer(opts: Record<string, unknown>): Promise<RtpEngineResult>;
  ...
  playDTMF(opts: Record<string, unknown>): Promise<RtpEngineResult>;
  subscribeDTMF(
    logger: Logger,
    callId: string,
    tag: string | null,
...
const { MediaPath } = require('./constants.json') as {
  MediaPath: { NoMedia: string; PartialMedia: string; FullMedia: string };
};

const debug = debugFactory('jambonz:sbc-outbound');

type SipErrorClass = new (status: number, reason?: string) => Error & { status: number };
type DrachtioStatics = typeof DrachtioSrf & {
  SipError: SipErrorClass;
  parseUri: (uri: string) => { user?: string; host?: string; params: Record<string, string | null | undefined> };
  stringifyUri: (uri: object) => string;
};
const D = DrachtioSrf as DrachtioStatics;
const SipError = D.SipError;
const parseUri = D.parseUri;
const stringifyUri = D.stringifyUri;

const makeInviteInProgressKey = (callid: string) => `sbc-out-iip${callid}`;
const IMMUTABLE_HEADERS = ['via', 'from', 'to', 'call-id', 'cseq', 'max-forwards', 'content-length'];

interface BLegFromParams {
  logger: Logger;
  req: SrfReq;
  host?: string;
  fromUser?: string;
...
const createBLegToHeader = (req: SrfReq, teams: boolean) => {
...
const initCdr = (req: SrfReq, invite: SrfReq) => {
  const {srf} = req;
  ...
  return {
    ...
    direction: 'outbound',
    host: (srf.locals as { sipAddress?: string }).sipAddress,
    remote_host: uri.host ?? '',
    trace_id: trace_id || '00000000000000000000000000000000'
  };
};

interface CarrierGatewayMeta {
  name: string;
...
type DialUriEntry = { private_network: boolean; uri: string };
...
class CallSession extends Emitter {
  ...
  logger!: Logger;
  useWss!: boolean;
  stats!: { increment: (name: string, tags?: string[]) => void };
  idleEmitter!: NodeJS.EventEmitter;
  activeCallIds!: Map<string, CallSession>;
  ...
  lookupCarrierByAccountLcr!: (accountSid: string, called: string) => Promise<string | null>;
  _mediaPath!: string;
  recordingNoAnswerTimeout!: number;
  _subscribedForDTMF?: boolean;
  teams?: boolean;
  offer!: RtpEngineClient['offer'];
  answer!: RtpEngineClient['answer'];
  del!: RtpEngineClient['del'];
  blockMedia!: RtpEngineClient['blockMedia'];
  unblockMedia!: RtpEngineClient['unblockMedia'];
  blockDTMF!: RtpEngineClient['blockDTMF'];
  unblockDTMF!: RtpEngineClient['unblockDTMF'];
  playDTMF!: RtpEngineClient['playDTMF'];
  subscribeDTMF!: RtpEngineClient['subscribeDTMF'];
  unsubscribeDTMF!: RtpEngineClient['unsubscribeDTMF'];
  subscribeRequest!: RtpEngineClient['subscribeRequest'];
  subscribeAnswer!: RtpEngineClient['subscribeAnswer'];
  unsubscribe!: RtpEngineClient['unsubscribe'];
  srsClients!: InstanceType<typeof SrsClient>[];
  rtpEngineOpts!: RtpEngineOptsBundle;
  rtpEngineResource!: { destroy: () => Promise<unknown> };
  contactHeader?: string;
  connectedUri?: string;
  uas?: SipDialog;
  uac?: SipDialog;
  recordingNoAnswerTimer?: ReturnType<typeof setTimeout> | null;
  isRecordingNoAnswerResponded?: boolean;

  constructor(logger: Logger, req: SrfReq, res: SrfRes) {
    super();
    this.req = req;
    this.res = res;
    this.srf = req.srf;
    this.logger = logger.child({ callId: req.get('Call-ID') });
    const reg = req.locals.registration as (RegistrationRecord & { protocol?: string }) | undefined;
    this.useWss = Boolean(reg && reg.protocol === 'wss');
    ...
    this.recordingNoAnswerTimeout = (Number(process.env.JAMBONES_RECORDING_NO_ANSWER_TIMEOUT) || 2) * 1000;
  }
  ...
  subscribeForDTMF(dlg: SipDialog) {
  ...
  async connect() {
    ...
    const engine = this.srf.locals.getRtpEngine() as RtpEngineClient | null;
    if (!engine) {
    ...
    let uris: DialUriEntry[] = [];
    const mapGateways = new Map<string, CarrierGatewayMeta>();
    ...
    try {
      ...
      if (this.req.locals.registration) {
        ...
        if (this.req.has('X-Override-To')) {
          ...
          const parsedContact = parseUri(contact);
          parsedContact.user = dest;
          destUri = stringifyUri(parsedContact);
        ...
        if (this.req.has('X-SIP-Proxy')) {
          ...
          if (proxy !== undefined && !proxy.startsWith('sip:') && !proxy.startsWith('sips:')) proxy = `sip:${proxy}`;
        }
      }
      else if (teams) {
        ...
        try {
          ...
          let voip_carrier_sid: string | null = this.req.get('X-Requested-Carrier-Sid') || null;
          ...
          const goodGateways: SipGatewayRow[] = [];
          for (const g of gateways) {
            if (!(await isBlackListedSipGateway(
              client as unknown as RealtimeClient,
              this.logger,
              g.sip_gateway_sid
            ))) {
              goodGateways.push(g);
          ...
          if (gws.length) {
            ...
            gws.forEach((o) => {
              ...
              const obj: CarrierGatewayMeta = {
                name: carrier.name,
                diversion: carrier.diversion,
                hostport,
                transport,
                scheme,
                register_from_domain: carrier.register_from_domain
              };
              if (carrier.register_username && carrier.register_password) {
                obj.auth = {
                  username: carrier.register_username,
                  password: carrier.register_password
                };
              }
              if (carrier.requires_register && carrier.register_sip_realm && carrier.register_sip_realm.length > 0) {
                proxy = u;
                u = `${scheme}:${prefix}${prependPlus}${calledNumber}@${carrier.register_sip_realm};transport=${transport}`;
                this.logger.debug({uri: u}, `using outbound proxy for this registered trunk: ${proxy}`);
              } else if (carrier.outbound_sip_proxy && carrier.outbound_sip_proxy.length > 0) {
                if (carrier.outbound_sip_proxy.includes('sip:') || carrier.outbound_sip_proxy.includes('sips:')) {
                  proxy = carrier.outbound_sip_proxy;
                } else if (isValidDomainOrIP(carrier.outbound_sip_proxy)) {
                  proxy =
                    `${scheme}:${prefix}${prependPlus}${calledNumber}@${carrier.outbound_sip_proxy};transport=${transport}`;
                  this.logger.debug({uri: u}, `using outbound proxy for this trunk: ${proxy}`);
                }
              }
              mapGateways.set(u, obj);
              pendingUriStrings.push(u);
              ...
              if (o.protocol === 'tls/srtp') {
                ...
                this.rtpEngineOpts = makeRtpEngineOpts(this.req, false, true, Boolean(o.pad_crypto), true, o.remove_ice, o.dtls_off);
                encryptedMedia = true;
              }
            });
            /* Check private network for each gw */
            uris = await Promise.all(pendingUriStrings.map(async(uStr) => {
              return {
                private_network: await isPrivateVoipNetwork(uStr),
                uri: uStr
              };
            }));
      ...
      uris = uris.sort((a, b) => Number(a.private_network) - Number(b.private_network));
      ...
      while (uris.length) {
        ...
        const entry = uris.shift();
        if (!entry) break;
        const { private_network, uri } = entry;
        ...
        if (0 === uris.length) {
          try {
            ...
            if (obj && typeof obj.callId === 'string' && obj.cseq != null) {
              hdrs = {
                ...hdrs,
                'Call-ID': obj.callId,
                'CSeq': `${String(obj.cseq)} INVITE`
              };
            }
          } catch (err) {
        ...
        if (gw) {
          ...
          const parsedReqUri = parseUri(this.req.uri);
          const tr = parsedReqUri.params.transport != null ? String(parsedReqUri.params.transport) : undefined;
          hdrs = {
            ...hdrs,
          ...
          this.logger.info({ uri: parsedReqUri, p }, `sending INVITE${p}`);
        }
        ...
        try {
          const {uas, uac} = await this.srf.createB2BUA(
            ...
            uri,
            {
              ...
              responseHeaders: (uacRes: Record<string, unknown>, headers: Record<string, string>) => {
                headers['X-CID'] = (uacRes as { get(name: string): string }).get('Call-ID');
                return headers;
              },
              headers: hdrs,
              auth: gw ? gw.auth : undefined,
              localSdpB: response.sdp,
              localSdpA: async(sdp: string, res: JambonzSrfResponse) => {
                const toHdr = res.getParsedHeader('To') as { params: { tag?: string } };
                this.rtpEngineOpts.uac.tag = toHdr.params.tag != null ? String(toHdr.params.tag) : null;
                ...
                const ansRes = await this.answer(ansOpts);
                this.logger.debug({answer: ansOpts, response: ansRes}, 'rtpengine answer');
                if ('ok' !== ansRes.result) {
                /* note: this can happen if call was abandoned while we were waiting for B leg to answer */
                  this.logger.info(`rtpengine answer failed with ${JSON.stringify(ansRes)}`);
                  throw new Error(`rtpengine failed: ${ansRes['error-reason']}`);
                }
                if (!ansRes.sdp) {
                  throw new Error('rtpengine answer: missing sdp');
                }
                return ansRes.sdp;
              }
            }, {
              cbRequest: async(err: unknown, inv: SrfReq) => {
                ...
                const cseqHdr = inv.getParsedHeader('CSeq') as unknown as { seq: number };
                const opts = {
              ...
              cbProvisional: (response: SrfRes) => {
                const pr = response as SrfRes & { status: number; body?: string };
                if (!earlyMedia && [180, 183].includes(pr.status) && pr.body) earlyMedia = true;
              }
            } as unknown as NonNullable<Parameters<DrachtioSrf['createB2BUA']>[4]>
          );
          ...
          const sipE = err instanceof SipError ? err : null;
          if (uris.length === 0 ||          // exhausted all targets
            ...
            sipE === null ||   // unexpected error
            sipE.status === 487) {           // caller hung up

            const abandoned = err instanceof Error && err.message.includes('rtpengine failed: Unknown call-id');
            const status = sipE?.status ?? (abandoned ? 487 : 500);
            if (sipE) this.logger.info(`final call failure  ${status}`);
            else if (!abandoned) {
            ...
            this.logger.debug(`got final outdial error: ${String(err)}`);
            ...
            const se = err instanceof SipError ? err : null;
            this.logger.info(`got ${se?.status}, cranking back to next destination`);
          }
        }
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : '';
      if ('abandonded' !== msg) this.logger.error({ err }, `Error setting up outbound call to: ${uris}`);
      this.emit('failed');
      this.srf.endSession(this.req);
      this.rtpEngineResource.destroy();
    }
  ...
  _setHandlers({ uas, uac }: { uas: SipDialog; uac: SipDialog }) {
    ...
    this.uac = uac;
    [uas, uac].forEach((dlg) => {
      dlgOn(dlg, 'destroy', async(byeUnknown: unknown) => {
        const bye = byeUnknown as SrfReq;
        ...
        try {
          ...
          Object.keys(bye.headers).forEach((h) => {
            if (!IMMUTABLE_HEADERS.includes(h)) {
              const v = bye.headers[h];
        ...
        (dlg as unknown as { other: SipDialog | null }).other = null;
        (other as unknown as { other: SipDialog | null }).other = null;
    ...
    dlgOn(uas, 'modify', this._onReinvite.bind(this, uas) as (...args: unknown[]) => void);
    dlgOn(uac, 'modify', this._onReinvite.bind(this, uac) as (...args: unknown[]) => void);

    dlgOn(uas, 'refer', this._onFeatureServerTransfer.bind(this, uas) as (...args: unknown[]) => void);
    dlgOn(uac, 'refer', this._onRefer.bind(this, uac) as (...args: unknown[]) => void);

    dlgOn(uas, 'info', this._onInfo.bind(this, uas) as (...args: unknown[]) => void);
    dlgOn(uac, 'info', this._onInfo.bind(this, uac) as (...args: unknown[]) => void);

    // default forwarding of other request types
    forwardInDialogRequests(uac,  ['notify', 'options', 'message']);
  }

  _startRecordingNoAnswerTimer(res: SrfRes) {
  ...
  async _onRefer(dlg: SipDialog, req: SrfReq, res: SrfRes) {
  ...
  async _onDTMF(dlg: SipDialog, payload: { event: number | string; duration: number | string }) {
    this.logger.info({payload}, '_onDTMF');
    try {
      let dtmf;
      const ev = typeof payload.event === 'number' ? payload.event : parseInt(String(payload.event), 10);
      switch (ev) {
        case 10:
          dtmf = '*';
          break;
...
Duration=${String(payload.duration)} `
  ...
  async _onReinvite(dlg: SipDialog, req: SrfReq, res: SrfRes) {
    try {
      ...
      if (isFullMediaRelease) {
        ...
        const answerSdp = await dlg.other.modify(a_sdp as string);
      ...
      if (process.env.JAMBONES_VIDEO_CALLS_ENABLED_IN_FS && responseSdp) {
      ...
      const st = typeof err === 'object' && err !== null && 'status' in err ? Number((err as { status: number }).status) : 500;
      res.send(st || 500);
      this.logger.error(err, 'Error handling reinvite');
    }
  }

  async _onInfo(dlg: SipDialog, req: SrfReq, res: SrfRes) {
    try {
      ...
      if (dlg.type === 'uas' && req.has('X-Reason')) {
        ...
        if (reason.startsWith('mute')) {
          ...
          if (reason === 'startCallRecording') {
            ...
            this.srsClients = arr.map((url: string) => new SrsClient(this.logger, {
              srf: this.srf,
              direction: 'outbound',
              ...
              accountSid,
              applicationSid,
              rtpEngineOpts: this.rtpEngineOpts as unknown as Record<string, unknown>,
              toTag,
              aorFrom,
              aorTo,
        ...
        const response = (await dlg.other.request(
          asDialogRequest({ method: 'INFO', headers, body: req.body })
        )) as JambonzSrfResponse;
  ...
  async _onFeatureServerTransfer(dlg: SipDialog, req: SrfReq, res: SrfRes) {
    try {
      ...
      this.uas = uac;
      (uac as SipDialog & { type: string }).type = 'uas';
      uac.other = this.uac!;
      this.uac!.other = uac;
      dlgOn(uac, 'info', this._onInfo.bind(this, uac) as (...args: unknown[]) => void);
      dlgOn(uac, 'modify', this._onReinvite.bind(this, uac) as (...args: unknown[]) => void);
      dlgOn(uac, 'refer', this._onFeatureServerTransfer.bind(this, uac) as (...args: unknown[]) => void);
      dlgOn(uac, 'destroy', (async() => {
        ...
        if (this.req.locals.cdr) {
          ...
          recording_url += `/${day.getFullYear()}/${(day.getMonth() + 1).toString().padStart(2, '0')}`;
          recording_url += `/${day.getDate().toString().padStart(2, '0')}/${record_format}`;
          const answeredAt = this.req.locals.cdr.answered_at ?? now;
          this.writeCdrs({
            ...this.req.locals.cdr,
            terminated_at: now,
            ...
            sip_status: 200,
            answered: true,
            duration: Math.floor((now - answeredAt) / 1000),
            ...(recordAllCalls ? { recording_url } : {})
          }).catch((err) => this.logger.error({ err }, 'Error writing cdr for completed call'));
        }
        this.srf.endSession(this.req);
      ...
      void (dlg as unknown as { destroy(): Promise<void> }).destroy().catch(() => {});
