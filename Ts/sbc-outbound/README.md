...
This directory is the **TypeScript** port: source lives in `.ts` files; production runs compiled output in `dist/` (same runtime behavior as the JavaScript `app.js` version).

## Build

```bash
npm ci
npm run build    # emits dist/ and copies lib/constants.json + data/*.json into dist/
npm start        # node dist/app.js
```

Develop locally with `npm run dev` (`tsx app.ts`).
...
Typically, this application runs under [pm2](https://pm2.io) using an [ecosystem.config.js](https://pm2.keymetrics.io/docs/usage/application-declaration/) file similar to this (use the compiled entrypoint after `npm run build`):
...
The included `Dockerfile` runs `npm run build` in a build stage and starts with `node dist/app.js` (production dependencies only in the final image).
